...
| B23 | **The deliverable a buyer paid for is unreachable — by anyone** | found by running a second paid job (`56588`) and then trying to *collect* it, which the first run never did. `submit` publishes `deliverable_url = http://localhost:9000/erc8183/job/56588/response`, but this seller runs `serve_a2a`, which never mounts `/erc8183` — the seller **404s its own advertised URL**, and `[storage].kind = "local"` wrote no file either (no `.agent-data` exists). The buyer holds a `SUBMITTED` job, has paid, and has no route to the goods; `bag erc8183 fetch` falls back to a log scan the public RPC rate-limits. Not localhost-vs-public — structurally unreachable | open (**G15**), needs IPFS storage; recorded here because "SUBMITTED on-chain" reads like success and is not |
...
| **G15** | **`deliverable_url` resolves for nobody** — see B23. The URL names an `/erc8183` mount `serve_a2a` does not serve, and `local` storage writes no file, so the paid deliverable exists only in the seller process. This is the single thing standing between "the payment rail works" and "the product is sellable" | G9 (IPFS storage) |
